...
# ─── INTENT DETECTION ─────────────────────────────────────────────────────────
_ANALYSIS_KEYWORDS = [
    "analyze", "analysis", "report", "excel", "roast", "red flag", "debt",
    "verdict", "profit", "revenue", "margin", "ratio", "cash flow", "cashflow",
    "balance sheet", "income", "financial", "forecast", "compare", "benchmark",
    "risk", "performance", "kpi", "ebitda", "roe", "roa", "fcf", "working capital",
    "liquidity", "solvency", "trend", "anomaly", "summary", "overview", "insights",
    "highlight", "worst", "best", "segment", "product", "region", "category",
    "sales", "cost", "expense", "loss", "asset", "liability", "equity", "chart",
    "graph", "dashboard", "download", "generate", "give me", "show me",
    "what happened", "why is", "how is", "explain the", "break down",
]

_CASUAL_PATTERNS = [
    "hello", "hi ", "hey", "how are you", "what can you do", "who are you",
    "what are you", "nice to meet", "thanks", "thank you", "ok ", "okay",
    "sure", "great", "good job", "cool", "awesome", "interesting", "got it",
    "i see", "makes sense", "what do you think", "tell me about yourself",
    "can you help", "what should i", "what would you", "what is your",
...
def _is_casual(text: str) -> bool:
    t = text.lower().strip()
    if len(t) < 30 and not _is_analysis_request(t):
        return True
    return any(t.startswith(p) or p in t for p in _CASUAL_PATTERNS) and not _is_analysis_request(t)
...
defaults = {
    ...
    "df":             None,
    "tone":           "Analyst",
    "analysis_done":  False,   # ← True after first analysis completes
}
...
if user_input:
    ...
    # ── Decide what context to give the agent ──────────────────────────────
    # After initial analysis is done, only re-inject docs if this message
    # is genuinely asking for more analysis. Casual messages get no doc context
    # so the agent can just chat naturally without looping back to the report.
    analysis_requested = _is_analysis_request(user_input)
    casual_message     = _is_casual(user_input)

    if st.session_state.analysis_done and casual_message:
        # Pure conversation — no document context injected
        pass_docs     = None
        pass_doc_type = None
        spinner_text  = "Thinking…"
    elif st.session_state.docs_loaded:
        # Analysis context available and relevant
        pass_docs     = st.session_state.get("docs")
        pass_doc_type = st.session_state.get("doc_type")
        ...
        with st.spinner(spinner_text):
            try:
                response = run_agent(
                    ...
                    tone=st.session_state.tone,
                    docs=pass_docs,
                    doc_type=pass_doc_type,
                )
                st.markdown(response)

                # ── Excel download — only when explicitly requested ────────
                excel_keywords = ["excel", "download report", "generate report",
                                  "export report", "give me report", "send report",
                                  "create report", "make report"]
                wants_excel = any(w in user_input.lower() for w in excel_keywords)

                if wants_excel and st.session_state.get("df") is not None:
                    try:
                        path = generate_excel_report(
                            company_name=st.session_state.company_name or "Company",
                            df=st.session_state.get("df"),
                            doc_type=st.session_state.doc_type,
                            analysis_text=response,   # ← pass LLM response so engine uses real values
                        )
                        ...
                        st.error(f"Excel error: {excel_err}")

                # Mark analysis done if this was a fresh analysis
                if analysis_requested and st.session_state.docs_loaded:
                    st.session_state.analysis_done = True
