...
# ─── SAFE CONVERTERS ──────────────────────────────────────────────────────────
def safe_int(val):
    """Convert to int safely — returns original value if not numeric."""
    try:
...
def safe_float(val):
    """Convert to float safely — returns 0.0 if not numeric."""
    try:
        return float(val)
...
def detect_columns(df: pd.DataFrame) -> dict:
    ...
    # ── Safety check: numeric semantics must point to actually numeric columns ──
    # Without this, a string column (e.g. Country) can be mis-detected as
    # 'quantity' (via keyword 'count'), causing int() to fail on concatenated strings.
    _NUMERIC_SEMANTICS = {'sales', 'profit', 'quantity', 'ship_cost', 'discount'}
    numeric_col_set = set(result['_numeric'])
    for key in list(result.keys()):
        if key in _NUMERIC_SEMANTICS and result[key] not in numeric_col_set:
            del result[key]   # drop mis-detected non-numeric column
...
def build_overview(ws, df, company_name, doc_type, cols):
    ...
    if 'quantity' in cols:
        kpis.append(("Units Sold",    f"{safe_int(df[cols['quantity']].sum()):,}", True))
    ...
    def write_group(title, groupby_col, anchor, chart_type="bar"):
        ...
        for _, r in grp.iterrows():
            ...
            if 'quantity' in cols and 'Qty' in grp.columns:
                dat(ws, row, col, safe_int(r['Qty']), align="right")
            row += 1
        data_end = row - 1
...
def build_products(ws, df, cols):
    ...
    for _, r in top20.iterrows():
        ...
        if 'profit' in cols:
            ...
            ci += 1
        if 'quantity' in cols:
            dat(ws, row, ci, safe_int(r['Qty Sold']), align="right")
        row += 1
    top_end = row - 1
...
def build_shipping(ws, df, cols):
    ...
    if 'ship_mode' in cols:
        ...
        for _, r in mode_df.iterrows():
            ...
            dat(ws, row, 2, safe_int(r['Count']), align="right")
...
def generate_excel_report(
    ...
    analysis_text: str = None,   # LLM analysis passed from app.py (reserved for future use)
